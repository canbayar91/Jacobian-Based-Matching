...
int MeshBuilder::matchFace(FaceData* face) {
	...
	do {
		...
		double currentValue = calculateScaledJacobian(face, pair, runner);

		// Update the highest priority face if a more suitable face exists
		if (!pair->matched && currentValue > maxValue) {
...
void MeshBuilder::updatePriority(FaceData* face) {
	...
	if (!face->matched) {
		...
		// Initialize maximum metric value to -1 (minimum scaled jacobian value is 0)
		double maxValue = -1;
		...
		do {
			...
			if (!neighbor->matched) {
				...
				double currentValue = calculateScaledJacobian(face, neighbor, runner);
...
double MeshBuilder::calculateScaledJacobian(FaceData* face, FaceData* pair, HalfEdgeData* runner) {
